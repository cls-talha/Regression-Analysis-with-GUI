...
inline std::string predict(std::string year_value)
{
    ...
    std::ifstream my_csv;
    my_csv.open("Dataset\\Population.csv");
    ...
    std::ofstream ModelW("Trained_Model\\Trained_Para_Population_Dataset.txt", std::ofstream::out);
    if (ModelW.is_open())
    {
